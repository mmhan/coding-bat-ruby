module Warmup2
  ...
  # Example:
	...
	def string_match(a_str, b_str)
		result = 0
		return result if a_str.length < 2 || b_str.length < 2

		shorter = [a_str.length, b_str.length].min

		(0..shorter - 2).each do |i|
			result += 1 if a_str[i..i+1] == b_str[i..i+1]
		end
		result
	end
end